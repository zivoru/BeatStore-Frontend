...
class MyBeats extends Component {
    ...
    getPublishedBeats() {
        if (this.props.user !== null && this.props.user !== undefined && this.props.user !== "empty") {
            ...
            axios.get("/api/v1/beats/user/" + this.props.user.id + "?page=" + this.state.page + "&size=1000").then(res => {
                this.setState({
                    publishedBeats: res.data.totalElements === 0 ? "empty" : res.data.content,
                    loading: false
                })
            }).catch(() => {
                this.setState({
                    publishedBeats: "empty",
                    loading: false
                })
            })
            ...
            axios.get("/api/v1/beats/drafts").then(res => {
                this.setState({
                    draftBeats: res.data.length === 0 ? "empty" : res.data
                })
            }).catch(() => {
                this.setState({
                    draftBeats: "empty"
            ...
            axios.get("/api/v1/beats/sold").then(res => {
                this.setState({
                    soldBeats: res.data.length === 0 ? "empty" : res.data
                })
            }).catch(() => {
                this.setState({
                    soldBeats: "empty"
    ...
    deleteDraftBeat = () => {
        axios.delete("/api/v1/beats/" + this.state.deleteId).then(() => {
            setTimeout(() => this.setState({update: !this.state.update}), 100);
            setTimeout(() => this.draftView(), 200);
            setTimeout(() => this.setState({warningDraftDelete: false}), 300);
        }).catch()
    }
